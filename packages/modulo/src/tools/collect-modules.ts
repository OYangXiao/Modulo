...
export function collect_modules(kind: 'pages' | 'modules') {
    ...
    : readdirSync(module_path, { withFileTypes: true })
        ...
        .map((dirent) => {
          ...
          const entry_file_path = [
            ...
            ...(framework_name === 'vue'
              ? [resolve(dir_path, 'index.vue'), resolve(dir_path, 'main.vue'), resolve(dir_path, `${dirent.name}.vue`)]
              : []),
            ...
            debug_log(
              ...
              `${path} ${picocolors[exists ? 'green' : 'red'](`exists - ${exists}`)}`,
            );
            return exists;
          });
  ...
  return module_entries.length > 0 ? Object.fromEntries(module_entries) : undefined;
}
